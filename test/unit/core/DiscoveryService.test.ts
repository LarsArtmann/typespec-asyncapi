...
} from "../../../src/domain/emitter/DiscoveryService.js";
...
describe("DiscoveryService", () => {
  ...
  describe("discoverOperations", () => {
    ...
    it("should handle program without getGlobalNamespaceType method", async () => {
      ...
      const result = await Effect.runPromise(
        discoveryService.discoverOperations(programWithoutMethod),
      );
  ...
  describe("executeDiscovery", () => {
    it("should execute complete discovery process", async () => {
      ...
      const result: DiscoveryResult = await Effect.runPromise(
        discoveryService.executeDiscovery(mockProgram),
      );
  ...
  describe("error handling", () => {
    it("should handle program with null getGlobalNamespaceType", async () => {
      ...
      const result = await Effect.runPromise(
        discoveryService.discoverOperations(programWithNullMethod),
      );
      expect(result).toHaveLength(0);
    });
