...
export async function compileAsyncAPISpecWithoutErrors(
	source: string,
	options: AsyncAPIEmitterOptions = {},
): Promise<{ outputFiles: Map<string, string | { content: string }>; program: Program; diagnostics: readonly Diagnostic[] }> {
	const result = await compileAsyncAPISpecRaw(source, options)
...
export async function parseAsyncAPIOutput(outputFiles: Map<string, string | {
	...
	try {
		...
		// Alpha debugging: Check if any AsyncAPI-related files exist at all
		const possibleAsyncAPIFiles = allFiles.filter(path =>
			path.toLowerCase().includes('asyncapi') || 
			path.includes('async') ||
			path.endsWith('.json') ||
			path.endsWith('.yaml')
		)
		Effect.log(`🔍 All possible AsyncAPI-related files: ${possibleAsyncAPIFiles.slice(0, 10).join(', ')}${possibleAsyncAPIFiles.length > 10 ? '...' : ''}`)
		
		// Check emitter output directory
		const emitterOutputFiles = allFiles.filter(path => 
			path.includes('@lars') || 
			path.includes('typespec-asyncapi') ||
			path.includes('tsp-output')
		)
		Effect.log(`📤 Emitter output files: ${emitterOutputFiles.slice(0, 10).join(', ')}${emitterOutputFiles.length > 10 ? '...' : ''}`)
