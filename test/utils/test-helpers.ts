...
export async function parseAsyncAPIOutput(outputFiles: Map<string, string | {
	...
	try {
		...
		// NO MORE ALPHA FALLBACK: Emitter works, so if file not found it's a real error
		const availableFiles = Array.from(outputFiles.keys())
		Effect.log(`❌ File not found: ${filename}`)
		Effect.log(`📁 Available files (${availableFiles.length}): ${availableFiles.slice(0, 5).join(', ')}...`)

		throw new Error(`AsyncAPI file ${filename} not found. Available files: ${availableFiles.join(", ")}`)
	} catch (error) {
		const availableFiles = Array.from(outputFiles.keys())
