...
export async function compileAsyncAPISpec(
	...
	try {
		...
		for (const [fileName, content] of outputFiles) {
			if ((fileName.includes('asyncapi') || fileName.includes('AsyncAPI')) && 
		...
		throw new Error(`No valid AsyncAPI files found in output. Found files: ${Array.from(outputFiles.keys()).join(', ')}`)
	} catch (error) {
