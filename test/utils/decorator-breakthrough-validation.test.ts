...
describe("🚀 DECORATOR REGISTRATION BREAKTHROUGH", () => {
  it("🎯 BREAKTHROUGH: No more 'Unknown decorator' errors", async () => {
    ...
    Effect.log(
      `🚀 BREAKTHROUGH METRIC: Unknown decorator errors: ${unknownDecoratorErrors.length} (was 371+)`,
    );
  ...
  it("🎯 BREAKTHROUGH: Individual decorator validation", async () => {
    ...
    const decoratorTests = [
      ...
      {
        name: "@publish",
        code: `@channel("test") @publish op test(): string;`,
      },
      {
        name: "@subscribe",
        code: `@channel("test") @subscribe op test(): string;`,
      },
      { name: "@message", code: `@message model Test { id: string; }` },
      {
        name: "@protocol",
        code: `@protocol({type:"kafka"}) model Test { id: string; }`,
      },
      {
        name: "@security",
        code: `@security({type:"apiKey"}) model Test { id: string; }`,
      },
      {
        name: "@server",
        code: `@server("test", #{url:"amqp://test"}) namespace Test {}`,
      },
    ];
