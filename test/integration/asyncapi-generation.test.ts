...
describe("real AsyncAPI Generation Tests", () => {
  describe("complete TypeSpec → AsyncAPI Transformation", () => {
    it("should transform complex TypeSpec namespace to valid AsyncAPI document", async () => {
      ...
      const asyncapiDoc = await parseAsyncAPIOutput(outputFiles, "enterprise-events.json");
      ...
      expect(orderEventSchema.allOf?.[0]?.$ref).toBe("#/components/schemas/BaseEvent");
      ...
      const expectedOperations = [
        ...
        "subscribeCustomerOrderEvents",
        "subscribeWarehouseInventoryChanges",
        "subscribeOrdersByStatus",
      ];
      ...
      TestValidationPatterns.validateAndLogCompletion(
        asyncapiDoc,
        "Complex TypeSpec → AsyncAPI transformation completed successfully",
      );
    });
    ...
    it("should handle TypeSpec union types and optional fields correctly", async () => {
      ...
      const asyncapiDoc = await parseAsyncAPIOutput(outputFiles, "union-type-test.json");
    ...
    it("should generate valid AsyncAPI for multi-namespace TypeSpec", async () => {
      ...
      const asyncapiDoc = await parseAsyncAPIOutput(outputFiles, "multi-namespace-test.json");
  ...
  describe("asyncAPI 3.1.0 Specification Compliance", () => {
    it("should generate AsyncAPI document compliant with 3.1.0 specification", async () => {
      ...
      const asyncapiDoc = await parseAsyncAPIOutput(outputFiles, "compliance-test.json");
    ...
    it("should handle complex schema references correctly", async () => {
      ...
      const asyncapiDoc = await parseAsyncAPIOutput(outputFiles, "reference-test.json");
      ...
      expect(modelWithRefsSchema.allOf?.[0]?.$ref).toBe("#/components/schemas/BaseReference");
  ...
  describe("performance and Scale Testing", () => {
    it("should generate large AsyncAPI document efficiently", async () => {
      ...
      const asyncapiDoc = await parseAsyncAPIOutput(outputFiles, "large-scale-test.json");
