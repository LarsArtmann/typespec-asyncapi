 ...
 */
Given(
  "I have a TypeSpec model with @channel decorator",
...
Given(
  ...
  async function (this: AsyncAPIWorld) {
    ...
    console.log(
      "🚧 BDD: Creating TypeSpec model with @securityEnhanced decorator",
    );
  },
);

Given(
  "I have a TypeSpec model with MQTT protocol binding",
  async function (this: AsyncAPIWorld) {
...
Given(
  "I have invalid TypeSpec decorator configuration",
  async function (this: AsyncAPIWorld) {
...
Then(
  "I should receive a valid AsyncAPI 3.0 document",
  async function (this: AsyncAPIWorld) {
    if (!world.isValidAsyncAPI()) {
      throw new Error(
        "Expected valid AsyncAPI 3.0 document, but got invalid or null document",
      );
    }
  },
);
...
Then(
  "document should contain be corresponding channel",
  async function (this: AsyncAPIWorld) {
...
Then(
  "channel should have correct operation bindings",
  async function (this: AsyncAPIWorld) {
