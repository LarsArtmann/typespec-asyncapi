...
export class AsyncAPIDocumentationValidator {
  ...
  async validateAsyncAPI(
    ...
    try {
      ...
      result.errors.push(
        `AsyncAPI parsing failed: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  ...
  validateStructure(
    asyncapi: AsyncAPIObject,
    expectations: AsyncAPIStructureExpectation,
  ): string[] {
    const errors: string[] = [];
    ...
    if (expectations.info) {
      if (!asyncapi.info) {
        ...
        if (
          expectations.info.description &&
          asyncapi.info.description !== expectations.info.description
        ) {
          errors.push(
            `Expected info.description "${expectations.info.description}", got "${asyncapi.info.description}"`,
          );
        }
      }
    }
    ...
    if (expectations.channelNames) {
      ...
      const missingChannels = expectations.channelNames.filter(
        (name) => !actualChannelNames.includes(name),
      );
    ...
    if (expectations.operationNames) {
      ...
      const missingOperations = expectations.operationNames.filter(
    ...
    if (expectations.componentSchemas) {
      ...
      const missingSchemas = expectations.componentSchemas.filter(
    ...
    if (expectations.componentMessages) {
      ...
      const missingMessages = expectations.componentMessages.filter(
        ...
        if (operation.channel) {
          ...
          if (typeof channelRef === "string" && channelRef.startsWith("#/channels/")) {
            ...
            if (!asyncapi.channels || !asyncapi.channels[channelName]) {
              errors.push(
                `Operation '${operationId}' references non-existent channel '${channelName}'`,
              );
            }
          }
        }
