...
const asyncAPIValidationUtils = {
  ...
  extractErrors: (spec: unknown): Array<{ path: string; message: string }> => {
    ...
    if (!asyncAPISpec.asyncapi) {
      ...
      errors.push({
        path: "asyncapi",
        message: `Unsupported AsyncAPI version: ${asyncAPISpec.asyncapi}`,
      });
    }
 ...
 */
export function validateAsyncAPIMessage(
  message: unknown,
 ...
 */
export function validateAsyncAPIChannel(
  channel: unknown,
): Effect.Effect<ValidationResult, Error, never> {
  return Effect.gen(function* () {
    const errors: Array<{ path: string; message: string; severity: "error" | "warning" | "info" }> =
...
export class AsyncAPIValidator {
  ...
  static batchValidate(
    specs: Array<{ name: string; data: unknown }>,
  ): Effect.Effect<Array<{ name: string; result: ValidationResult }>, Error, never> {
    return Effect.gen(function* () {
      const results: Array<{ name: string; result: ValidationResult }> = [];

      for (const spec of specs) {
        const result = yield* Effect.catchAll(validateAsyncAPISpec(spec.data), (error) =>
          Effect.succeed({
            valid: false,
            errors: [
              {
