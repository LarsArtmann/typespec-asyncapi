...
- **CLI Usage**: Corrected TypeSpec CLI invocation from `pnpm dlx tsp` to `tsp compile` with proper paths
...
- Use `tsp compile <directory>` instead of `pnpm dlx tsp compile <file>`
