...
| 8 | Golden-file locks added | `test/golden/lock-fixtures.test.ts` byte-locks three previously-unlocked outputs: named-union `oneOf` (`TextContent | ImageContent`), template instantiation (`Page<User>` → `PageUser` ref chain), `@protocol` kafka/http/ws bindings. 3 `.expected.yaml` files generated from the compiler (temp generator script deleted after use). |
