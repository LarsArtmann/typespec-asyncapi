...
| **#214** | Test framework pnpm dlx/tsp CLI deps                     | Tests now use programmatic `compileAndDiagnose()` via `test/utils/test-helpers.ts` — no process spawning. Latest comment: "FULLY RESOLVED".                                          |
