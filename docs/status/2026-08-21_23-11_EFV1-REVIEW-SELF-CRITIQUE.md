...
| 3   | The two metric errors I fixed were **written into ROADMAP.md by a prior session today** (2026-08-21 planning docs) without primary-source verification — the exact failure mode the global "verify-external-claims" lesson warns about, now proven to apply to _internal_ doc claims too                                                                                                                                                                                                                                                                                                                                                                        | Low (fixed)       | Metrics hand-typed from memory/estimate at write time                                                                              | See e.1                                                                                                |
...
2. **v0.4.0 sequencing:** EFv1 is confirmed terminal-but-alive (not deprecated, `openapi3` still depends on it, trigger unfired). Is the direct-AST rewrite still the v0.4.0 _headline_, or would you rather sequence it behind other v0.4.0 items and keep it purely trigger-driven? Strategy call, not a code question.
...
_Point-in-time snapshot. Annotate, never rewrite, when superseded. Format note: written as Markdown per explicit user request, overriding this report type's HTML default._
