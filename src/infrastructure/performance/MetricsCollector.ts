 ...
 */
export const MetricsCollector = Context.GenericTag<
  "MetricsCollector",
  MetricsCollector
>("MetricsCollector");
...
export const MemoryMetricsCollector = {
    ...
    Effect.gen(function* () {
      ...
      if (!startTime) {
        yield* Effect.logWarning(
          `⚠️ No start time found for operation: ${operation}`,
        );
        return null;
      }
  ...
  recordMetrics: (metrics: PerformanceMetrics) =>
    Effect.sync(() => {
      ...
      if (
        metricsHistory.length > PERFORMANCE_MONITORING.METRICS_HISTORY_LIMIT
      ) {
        metricsHistory.splice(
          0,
          metricsHistory.length - PERFORMANCE_MONITORING.METRICS_HISTORY_LIMIT,
        );
      }
  ...
  getAggregatedMetrics: () =>
    Effect.sync(() => {
      ...
      const operationCounts = metricsHistory.reduce(
        (acc, m) => {
      ...
      const totalDuration = metricsHistory.reduce(
        (sum, m) => sum + m.duration,
        0,
      );
      const avgMemory =
        metricsHistory.reduce((sum, m) => sum + m.memoryUsage, 0) /
...
export const PerformanceMonitor = {
  ...
  incrementDocumentsProcessed: () =>
    Effect.sync(() => {
      globalThis.__ASYNCAPI_DOCUMENTS_PROCESSED =
        (globalThis.__ASYNCAPI_DOCUMENTS_PROCESSED ?? 0) + 1;
    }),
};
