...
export interface ErrorHandler {
  ...
  readonly handleSchemaError: (error: SchemaError) => Effect.Effect<void, never>
...
export const CentralizedErrorHandler: ErrorHandler = {
  handleError: (error) =>
    Effect.gen(function* () {
      ...
      if (error.severity === ErrorSeverity.CRITICAL) {
        yield* Effect.logError(`🚨 CRITICAL ERROR: ${error.message}`, { error })
      }
    }),
  
  handleCompilationError: (error) =>
    Effect.gen(function* () {
      yield* Effect.logError(`❌ TypeSpec Compilation Error: ${error.message}`, {
        error_id: error.id,
  ...
  getErrorSummary: () =>
    Effect.sync(() => {
      ...
      const errorsBySeverity = errorRegistry.reduce((acc, error) => {
      ...
      const recentErrors = errorRegistry
        .slice(-10)
        .reverse()
...
export const ErrorFactory = {
  compilationError: (message: string, context: Partial<CompilationError>): CompilationError => ({
  ...
  pluginError: (message: string, context: Partial<PluginError>): PluginError => ({
  ...
  schemaError: (message: string, context: Partial<SchemaError>): SchemaError => ({
