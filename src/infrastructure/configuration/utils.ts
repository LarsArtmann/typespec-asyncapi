...
} from "../../domain/models/serialization-format-option.js";
    ...
    Effect.catchAll((error) =>
      Effect.gen(function* () {
        yield* Effect.logWarning(
          "⚠️  Effect.TS Schema conversion failed, falling back to manual JSON Schema:",
          error,
        );
        ...
        return {
          ...
          properties: {
            ...
            "file-type": {
              type: "string",
              enum: SERIALIZATION_FORMAT_OPTIONS,
              nullable: true,
            },
            "asyncapi-version": {
              type: "string",
              enum: [ASYNCAPI_VERSIONS.CURRENT],
              nullable: true,
            },
 ...
 */
export const createAsyncAPIEmitterOptions = (
  input: Partial<AsyncAPIEmitterOptions> = {},
) =>
  Effect.gen(function* () {
 ...
 */
export const isAsyncAPIEmitterOptions = (
  input: unknown,
): input is AsyncAPIEmitterOptions => {
    ...
    Effect.gen(function* () {
      ...
      .pipe(
        Effect.catchAll((error) =>
          Effect.gen(function* () {
            yield* Effect.logWarning(
              `⚠️  AsyncAPI options validation failed: ${String(error)}`,
            );
            return false;
          }),
        ),
