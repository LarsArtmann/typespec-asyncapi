...
export class ProtocolBindingFactory {
   ...
   */
  static createServerBindings(
    protocol: ProtocolType,
    config: Record<string, unknown> = {},
  ): ServerBindingsObject | undefined {
    switch (protocol) {
      case "kafka":
        return {
   ...
   */
  static createChannelBindings(
    protocol: ProtocolType,
    config: Record<string, unknown> = {},
  ): ChannelBindingsObject | undefined {
    switch (protocol) {
      case "kafka": {
        const kafkaConfig = config.topic
          ? (config as KafkaChannelBindingConfig & {
              topic: string;
            })
   ...
   */
  static createOperationBindings(
    protocol: ProtocolType,
    config: Record<string, unknown> = {},
  ): OperationBindingsObject | undefined {
    switch (protocol) {
      case "kafka":
        return {
   ...
   */
  static createMessageBindings(
    protocol: ProtocolType,
    config: Record<string, unknown> = {},
  ): MessageBindingsObject | undefined {
    switch (protocol) {
      case "kafka":
        return {
