...
export const httpPlugin: ProtocolPlugin = {
  ...
  generateOperationBinding: (operation: unknown) =>
    Effect.gen(function* () {
      ...
      const config =
        (operation as { config?: HttpOperationBindingConfig })?.config ?? {};
  ...
  generateMessageBinding: (message: unknown) =>
    Effect.gen(function* () {
      ...
      const config =
        (message as { config?: HttpMessageBindingConfig })?.config ?? {};
  ...
  validateConfig: (config: unknown) =>
    Effect.gen(function* () {
      ...
      if ("statusCode" in httpConfig && httpConfig.statusCode) {
        if (
          typeof httpConfig.statusCode !== "number" ||
          httpConfig.statusCode < 100 ||
          httpConfig.statusCode > 599
        ) {
          return false;
        }
      }
