 ...
 */
export const registerBuiltInPlugins = (): Effect.Effect<
  void,
  StandardizedError
> =>
  Effect.gen(function* () {
      ...
      Effect.catchAll((error) =>
        Effect.fail(
          createError({
            ...
            reassure:
              "The emitter will continue without Kafka protocol support",
            ...
            escape:
              "Use other protocol plugins or manual binding configuration",
            severity: "warning",
      ...
      Effect.catchAll((error) =>
        Effect.fail(
          createError({
            ...
            reassure:
              "The emitter will continue without WebSocket protocol support",
            why: "Plugin registration failed due to configuration or dependency issues",
...
export const generateProtocolBinding = (
  ...
  Effect.gen(function* () {
    ...
    yield* Effect.log(
      `⚠️  Plugin ${protocolName} does not support ${bindingType} bindings`,
    );
    return null;
  });
