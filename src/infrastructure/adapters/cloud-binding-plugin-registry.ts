...
import type {
  ...
  AsyncAPIObject,
  CloudBindingResult,
} from "./cloud-binding-shared-types.js";
...
export class CloudBindingPluginRegistry {
  ...
  register(plugin: CloudBindingPlugin): void {
    ...
    Effect.log(
      `📦 Registered cloud binding plugin: ${plugin.name} (${plugin.bindingType})`,
    );
  }
    ...
    return Effect.gen(function* () {
      ...
      if (!plugin) {
        return yield* Effect.fail(
          new Error(`Unsupported binding type: ${bindingType}`),
        );
      }
      ...
      if (plugin.isCompatible && !plugin.isCompatible(target)) {
        return yield* Effect.fail(
          new Error(
            `Target ${target.name} is not compatible with ${bindingType} bindings`,
          ),
        );
      }
