...
export const kafkaPlugin: ProtocolPlugin = {
  ...
  generateOperationBinding: (operation: unknown) =>
    Effect.gen(function* () {
      ...
      const config =
        (operation as { config?: KafkaOperationBindingConfig })?.config ?? {};
  ...
  generateMessageBinding: (message: unknown) =>
    Effect.gen(function* () {
      ...
      const config =
        (message as { config?: KafkaMessageBindingConfig })?.config ?? {};
      ...
      const binding: KafkaMessageBinding = {
        ...
        schemaIdLocation:
          config.schemaIdLocation ??
          (PROTOCOL_DEFAULTS.kafka.schemaIdLocation as "header" | "payload"),
        schemaIdPayloadEncoding:
          config.schemaIdPayloadEncoding ?? "apicurio-new",
        ...(config.schemaLookupStrategy
  ...
  generateServerBinding: (server: unknown) =>
    Effect.gen(function* () {
      ...
      const config =
        (server as { config?: Partial<KafkaServerBinding> })?.config ?? {};
   ...
   */
  generateChannelBinding: (channel: unknown) =>
    Effect.gen(function* () {
      ...
      const channelData = channel as
        | { config?: KafkaChannelBindingConfig & { topic: string } }
        | Record<string, unknown>;
      const rawConfig =
        ("config" in channelData ? channelData.config : {}) ?? {};
  ...
  validateConfig: (config: unknown) =>
    Effect.gen(function* () {
      ...
      if ("groupId" in kafkaConfig && kafkaConfig.groupId) {
        if (
          typeof kafkaConfig.groupId !== "string" &&
          typeof kafkaConfig.groupId !== "object"
        ) {
          return false;
        }
      }
      ...
      if ("topic" in kafkaConfig && kafkaConfig.topic) {
        if (
          ...
          kafkaConfig.topic.trim().length === 0
        ) {
          return false;
        }
      }
      ...
      if ("partitions" in kafkaConfig && kafkaConfig.partitions) {
        if (
          typeof kafkaConfig.partitions !== "number" ||
          kafkaConfig.partitions < 1
        ) {
          return false;
        }
      }
      ...
      if ("replicas" in kafkaConfig && kafkaConfig.replicas) {
        if (
          typeof kafkaConfig.replicas !== "number" ||
          kafkaConfig.replicas < 1
        ) {
          return false;
        }
      }
