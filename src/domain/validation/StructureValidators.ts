...
export const validateChannels = (
  ...
  channelNames.forEach((channelName) => {
    ...
    if (isReference(channel)) {
      warnings.push(
        `Channel '${channelName}' is a reference ($ref) - cannot validate structure`,
      );
      return;
    }
...
export const validateOperations = (
  ...
  operationNames.forEach((operationName) => {
    ...
    if (isReference(operation)) {
      warnings.push(
        `Operation '${operationName}' is a reference ($ref) - cannot validate structure`,
      );
      return;
    }
    ...
    if (!operation.action) {
      errors.push(
        `Operation '${operationName}' missing required 'action' field`,
      );
    } else if (!["send", "receive"].includes(operation.action)) {
      errors.push(
        `Operation '${operationName}' has invalid action '${operation.action}', must be 'send' or 'receive'`,
      );
    }

    if (!operation.channel) {
      errors.push(
        `Operation '${operationName}' missing required 'channel' reference`,
      );
    }
  });
...
export const validateComponents = (
  ...
  if (doc.components.messages) {
    ...
    Object.entries(doc.components.messages).forEach(
      ([messageName, message]) => {
        // Skip validation for references
        if (isReference(message)) {
          warnings.push(
            `Message '${messageName}' is a reference ($ref) - cannot validate structure`,
          );
          return;
        }
