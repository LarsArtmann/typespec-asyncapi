...
type StandardizedError =
  import("../../utils/standardized-errors.js").StandardizedError;
...
export class AsyncAPIValidator {
  ...
  initializeEffect(): Effect.Effect<void, never> {
    ...
    return Effect.gen(function* () {
      ...
      yield* railwayLogging.logInitialization(
        "AsyncAPI 3.0.0 Validator with REAL @asyncapi/parser",
      );
      yield* railwayLogging.logInitializationSuccess(
        "AsyncAPI 3.0.0 Validator",
      );
    });
  }
  ...
  validateEffect(
    ...
    _identifier?: string,
  ): Effect.Effect<ExtendedValidationResult, Error, never> {
    ...
    return Effect.gen(function* () {
      ...
      if (
        ...
        inputDocument !== null &&
        "asyncapi" in inputDocument
      ) {
        const version = String(
          (inputDocument as Record<string, unknown>)["asyncapi"],
        );
        ...
        if (version !== "3.0.0") {
          const versionError = createError({
            ...
            reassure:
              "Version validation ensures compatibility with AsyncAPI 3.0.0 parser",
            ...
            escape:
              "Use a different validator that supports version ${version}",
            severity: "error",
      ...
      const parseResult = yield* Effect.tryPromise({
        try: () => {
          ...
          const parserError = createError({
            ...
            reassure:
              "This may be a syntax error or unsupported feature in your AsyncAPI document",
      ...
      if (
        ...
        parseResult !== null &&
        "document" in parseResult
      ) {
        const document = (parseResult as { document: unknown }).document;
        const diagnostics =
          (parseResult as { diagnostics?: unknown[] }).diagnostics ?? [];

        const duration = performance.now() - startTime;
        const metrics = document
          ? {
              duration,
              channelCount: Object.keys(document).filter((k) =>
                k.includes("channel"),
              ).length,
              operationCount: Object.keys(document).filter((k) =>
                k.includes("operation"),
              ).length,
              schemaCount: Object.keys(document).filter((k) =>
                k.includes("schema"),
              ).length,
              validatedAt: new Date(),
            }
        ...
        if (diagnostics.length === 0) {
          ...
          const errors = diagnostics
            .filter(
              (d: unknown) =>
                Number((d as { severity?: number }).severity) === 0,
            )
            .map((d: unknown) => {
              const diag = d as {
                message?: string;
          ...
          const warnings = diagnostics
            .filter(
              (d: unknown) =>
                Number((d as { severity?: number }).severity) === 1,
            )
            .map((d: unknown) => ({
              message: String(
                (d as { message?: string }).message ?? "Unknown warning",
              ),
  ...
  async validate(
    ...
    return Effect.runPromise(
      Effect.catchAll(
        effect,
        (error): Effect.Effect<ExtendedValidationResult, never> =>
          Effect.succeed({
            ...failure([
              {
