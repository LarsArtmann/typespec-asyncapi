  ...
  if (!fieldValue) {
    yield* Effect.fail(
      createError({
        ...
        code: `MISSING_INFO_${fieldName.toUpperCase()}` as
          | "MISSING_INFO_TITLE"
          | "MISSING_INFO_VERSION",
        context: {
          hasInfo: !!document.info,
          infoKeys: Object.keys(document.info || {}),
        },
      }),
    );
  }
...
export class DocumentGenerator {
  ...
  serializeDocument(
    ...
    return Effect.gen(function* () {
      yield* Effect.log(
        `📄 DocumentGenerator: Serializing to ${format.toUpperCase()}`,
      );
  ...
  serializeWithOptions(
    ...
    return Effect.gen(function* () {
      yield* Effect.log(
        `📄 Serializing AsyncAPI document with options: ${JSON.stringify(options)}`,
      );

      // Validate document structure before serialization
      yield* Effect.succeed(
        yield* new DocumentGenerator().validateDocumentStructure(document),
      );

      // Generate statistics
      const stats = yield* new DocumentGenerator().generateDocumentStats(
        document,
      );
  ...
  private validateDocumentStructure(
    ...
    return Effect.gen(function* () {
      ...
      if (!document.asyncapi) {
        yield* Effect.fail(
          createError({
            ...
            reassure:
              "This is a document structure issue that can be easily fixed",
            ...
            escape:
              "Use a default AsyncAPI version or check document creation process",
            severity: "error" as const,
      ...
      if (!document.info) {
        yield* Effect.fail(
          createError({
            ...
            escape:
              "Use DocumentBuilder.createInitialDocument to ensure proper structure",
            severity: "error" as const,
            code: "MISSING_INFO_SECTION",
            context: {
              documentProvided: !!document,
              hasAsyncapi: !!document.asyncapi,
            },
          }),
        );
      }
      ...
      yield* validateRequiredInfoField(document, "title", {
        ...
        escape:
          "Use a generic title like 'AsyncAPI Specification' as placeholder",
      });
      ...
      if (!versionPattern.test(document.asyncapi)) {
        yield* Effect.logWarning(
          `⚠️ AsyncAPI version '${document.asyncapi}' may not be valid semantic version`,
        );
      }
      ...
      if (
        document.operations &&
        Object.keys(document.operations).length === 0
      ) {
  ...
  private generateDocumentStats(
    document: AsyncAPIObject,
  ): Effect.Effect<DocumentStats, StandardizedError> {
    return Effect.gen(function* () {
    ...
    return Effect.gen(function* () {
      ...
      const optimized = yield* railway.trySync(
        () => {
          ...
          const documentGenerator = new DocumentGenerator();
          documentGenerator.removeEmptyObjects(
  ...
  validateSerializedContent(
    ...
    return Effect.gen(function* () {
      ...
      if (!content || content.trim().length === 0) {
        yield* Effect.fail(
          createError({
            ...
            reassure:
              "This is usually a serialization issue that can be debugged",
      ...
      if (format === "json") {
        ...
        const isValidJson = yield* railway
          .trySync(
            () => {
              JSON.parse(content);
              return true;
      ...
      if (format === "yaml") {
        ...
        if (content.includes("\t")) {
          yield* Effect.logWarning(
            `⚠️ YAML contains tabs - may cause parsing issues`,
          );
        }
