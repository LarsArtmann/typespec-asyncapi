...
> =>
  Effect.gen(function* () {
    ...
    const operationInfo = {
      ...
      description:
        getDoc(program, operation) ??
        `Generated from TypeSpec operation: ${operation.name}`,
    ...
    const parameters = operation.parameters?.properties
      ? Array.from(operation.parameters.properties.entries()).map(
          ([paramName, paramModel]) => {
            const schemaType =
              paramModel.type.kind === "String"
                ? "string"
                : paramModel.type.kind === "Number"
                  ? "number"
                  : paramModel.type.kind === "Boolean"
                    ? "boolean"
                    : "string";
    ...
    const messageSchema = operation.returnType
      ? {
          type: "object",
          properties:
            operation.returnType.kind === "Model" &&
            operation.returnType.properties?.size > 0
              ? Object.fromEntries(
                  Array.from(operation.returnType.properties.entries()).map(
                    ([propName, propModel]) => [
                      propName,
                      propModel.type.kind === "String"
                        ? { type: "string" }
                        : propModel.type.kind === "Number"
                          ? { type: "number" }
                          : propModel.type.kind === "Boolean"
                            ? { type: "boolean" }
                            : { type: "string" },
                    ],
                  ),
...
export const processOperations = (
  ...
  Effect.gen(function* () {
    ...
    yield* Effect.log(
      `✅ Processed ${operations.length} operations successfully`,
    );
