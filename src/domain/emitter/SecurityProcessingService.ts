...
export const processSingleSecurityConfig = (
  ...
  Effect.gen(function* () {
    yield* railwayLogging.logDebugGeneration(
      "security-scheme",
      config.name,
      config,
    );
    ...
    const securityScheme: SecuritySchemeObject =
      createSecuritySchemeFromConfig(config);
...
export const processSecurityConfigs = (
  ...
  Effect.gen(function* () {
    yield* Effect.log(
      `🔐 Processing ${securityConfigs.length} security configurations with functional composition...`,
    );
    ...
    yield* Effect.log(
      `📊 Processed ${securityConfigs.length} security configurations successfully`,
    );
 ...
 */
const createSecuritySchemeFromConfig = (
  config: SecurityConfig,
): SecuritySchemeObject => {
