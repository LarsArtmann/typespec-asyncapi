...
import {
  ErrorHandler,
  ErrorFactory,
} from "../../infrastructure/errors/CentralizedErrorHandler.js";
...
const applyMutation = <T>(
  ...
  mutation: DocumentUpdate<T>,
) =>
  Effect.gen(function* () {
    const documentCopy = JSON.parse(JSON.stringify(document)) as Record<
      string,
      unknown
    >;
    ...
    const finalOldValue: T | undefined = (current as Record<string, T>)[
      finalKey
    ];
...
export type DocumentManager = {
  ...
  >;
  readonly mutateDocumentAtomic: <T>(
    mutations: DocumentUpdate<T>[],
  ) => Effect.Effect<
    AsyncAPIObject,
    Error,
    "MetricsCollector" | "ErrorHandler"
  ...
  readonly rollbackToVersion: (
    versionId: string,
  ) => Effect.Effect<AsyncAPIObject, Error>;
 ...
 */
export const DocumentManager = Context.GenericTag<
  "DocumentManager",
  DocumentManager
>("DocumentManager");
...
export const ImmutableDocumentManager: DocumentManager = {
  ...
  mutateDocument: <T>(mutation: DocumentUpdate<T>) =>
    Effect.gen(function* () {
      ...
      const currentDocument = JSON.parse(
        JSON.stringify(currentState.currentDocument),
      ) as AsyncAPIObject;
      ...
      const documentMutation = createDocumentMutation(
        mutation,
      ...
      const isValid = yield* ImmutableDocumentManager.validateDocument(
        mutationResult.document,
      );
      if (!isValid) {
        // Rollback on validation failure
        yield* ImmutableDocumentManager.rollbackToVersion(
          currentState.versionHistory[0]?.id ?? "initial",
        );
        yield* Effect.fail(
          new Error(
            "Document validation failed, rolled back to previous version",
          ),
        );
        return undefined as never;
      }
  ...
  mutateDocumentAtomic: <T>(mutations: DocumentUpdate<T>[]) =>
    Effect.gen(function* () {
      ...
      for (const mutation of mutations) {
        const mutationResult = yield* applyMutation<T>(
          currentDocument,
          mutation,
        );
        ...
        if (!mutationResult.success) {
          ...
          yield* errorHandler.handleCompilationError(
            ErrorFactory.compilationError(
              `Atomic mutation failed at step ${appliedMutations.length + 1}`,
              {
          ...
          yield* Effect.fail(
            new Error(`Atomic mutation failed: operation unsuccessful`),
          );
          return undefined as never;
        }
  ...
  rollbackToVersion: (versionId: string) =>
    Effect.gen(function* () {
      ...
      const targetVersion = currentState.versionHistory.find(
        (v) => v.id === versionId,
      );
      if (!targetVersion) {
      ...
      yield* updateDocumentState({
        ...
        mutationHistory: currentState.mutationHistory.slice(
          0,
          currentState.mutationHistory.findIndex((m) =>
            targetVersion.mutations.some((tm) => tm.id === m.id),
          ) + 1,
        ),
      });

      yield* Effect.logInfo(
        `🔄 Rolled back to version ${targetVersion.version} (${versionId})`,
      );
  ...
  getHealthMetrics: () =>
    Effect.gen(function* () {
      ...
      return {
        ...
        lastMutationTime:
          currentState.mutationHistory[currentState.mutationHistory.length - 1]
            ?.timestamp ?? 0,
      };
    }),
};
