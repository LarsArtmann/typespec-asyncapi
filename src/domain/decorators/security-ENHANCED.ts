...
const isModel = (value: unknown): value is Model => {
  return (
    ...
    value !== null &&
    "kind" in value &&
    (value as Model & { kind: string }).kind === "Model"
  );
};
...
const isOperation = (value: unknown): value is Operation => {
  return (
    ...
    value !== null &&
    "kind" in value &&
    (value as Operation & { kind: string }).kind === "Operation"
  );
};
 ...
 */
function validateSecurityScheme(scheme: SecurityScheme): {
  ...
  switch (scheme.type) {
    case "apiKey": {
      ...
      if (!ASYNCAPI_API_KEY_LOCATIONS.includes(apiKeyScheme.in)) {
        errors.push(
          `Invalid API key location: ${apiKeyScheme.in}. Must be one of: ${ASYNCAPI_API_KEY_LOCATIONS.join(", ")}`,
        );
      }
    ...
    case "http": {
      ...
      if (httpScheme.scheme === "bearer") {
        if (!httpScheme.bearerFormat) {
          warnings.push(
            "Bearer scheme should specify bearerFormat for clarity",
          );
        }
    ...
    case "httpApiKey": {
      ...
      if (!ASYNCAPI_API_KEY_LOCATIONS.includes(httpApiKeyScheme.in)) {
        errors.push(
          `Invalid HTTP API key location: ${httpApiKeyScheme.in}. Must be one of: ${ASYNCAPI_API_KEY_LOCATIONS.join(", ")}`,
        );
      }
...
export const $securityEnhanced = (
  ...
  const existingConfigs =
    Array.from(stateMap.entries())
      ...
      .map(([, value]) => value as SecurityConfig[])[0] ?? [];
  ...
  for (const warning of validation.warnings) {
    ...
    Effect.logWarning(
      `⚠️  Security scheme '${securityConfig.name}': ${warning}`,
    );
  }
  ...
  if (validation.secretFields.length > 0) {
    Effect.logInfo(
      `🔒 Security scheme '${securityConfig.name}' has ${validation.secretFields.length} secret fields: ${validation.secretFields.join(", ")}`,
    );
  }
...
export const getSecurityConfigurations = (
  program: Program,
): Map<Model | Operation, SecurityConfig[]> => {
...
export const processSecuritySchemes = (
  program: Program,
  asyncApiDoc: Record<string, unknown>,
) => {
  return Effect.gen(function* () {
    ...
    for (const [_target, configs] of securityConfigs) {
      for (const config of configs) {
        ...
        const schemeType =
          (config.scheme as { type?: string }).type ?? "unknown";
        yield* Effect.logInfo(
          `🔧 Processed security scheme: ${config.name} (${schemeType})`,
        );
      }
    }

    yield* Effect.logInfo(
      `✅ Security schemes processing complete: ${Object.keys(doc.components.securitySchemes).length} schemes`,
    );
  });
};
