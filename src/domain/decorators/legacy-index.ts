...
import { $securityEnhanced as $security } from "./security-ENHANCED.js";
...
export function createAsyncAPIDecorators(
  program: Program,
    ...
    if (!program) {
      return yield* failWith(
        createError({
          ...
          escape:
            "Check TypeSpec compiler setup or use a different decorator registration approach",
      ...
      .trySync(
        () => {
          ...
          if (!program.getGlobalNamespaceType) {
            return Effect.fail(
              new Error("Program does not have getGlobalNamespaceType method"),
            );
          }
      ...
      .pipe(
        Effect.mapError((error) =>
          createError({
            ...
            reassure:
              "This indicates a TypeSpec version or configuration issue",
            ...
            escape:
              "Use a different TypeSpec program instance or check compiler setup",
            severity: "error" as const,
            code: "MISSING_PROGRAM_METHOD",
            context: {
              programMethods: Object.keys(program),
              originalError: error,
            },
          }),
        ),
    ...
    yield* railway
      .trySync(
        () => {
          ...
          if (!ns) {
            return Effect.fail(
              new Error("getGlobalNamespaceType returned null/undefined"),
            );
          }
      ...
      .pipe(
        Effect.mapError((error) =>
          createError({
            ...
            escape:
              "Try reinitializing the TypeSpec program or check compiler configuration",
            severity: "error" as const,
        ...
        .trySync(
          () => {
            if (typeof fn !== "function") {
              return Effect.fail(
                new Error(`Decorator function ${name} is not a function`),
              );
            }
