...
export function $protocol(
  ...
  if (isTypeSpecModel(config)) {
    const modelProperties: RekeyableMap<string, ModelProperty> =
      config.properties;
 ...
 PROCESSING @protocol decorator on: ${target.kind} ${target.name || "unnamed"}`);
  ...
  const validationResult = validateProtocolBinding(
    actualConfig.protocol,
    actualConfig.binding,
  );
  ...
  Effect.log(
    `�  Protocol binding validation warnings:`,
  ...
  Effect.log(
    `✅ Validated protocol config for ${actualConfig.protocol}:`,
    actualConfig,
  );
  ...
  Effect.log(
    ` Successfully stored protocol config for ${target.kind} ${target.name || "unnamed"}`,
  );
  ...
  switch (protocol) {
    case "kafka": {
      ...
      if (kafkaBinding.schemaId && !kafkaBinding.schemaIdLocation) {
        warnings.push(
          "schemaId specified without schemaIdLocation - defaulting to 'payload'",
        );
      }
      if (kafkaBinding.schemaLookupStrategy && !kafkaBinding.schemaId) {
        warnings.push(
          "schemaLookupStrategy specified without schemaId - may not work as expected",
        );
      }
      break;
    }
    ...
    case "websocket": {
      ...
      if (wsBinding.method && String(wsBinding.method) !== "GET") {
        warnings.push(
          "WebSocket binding method should be GET for upgrade requests",
        );
      }
      break;
    }
    ...
    case "http": {
      ...
      if (
        httpBinding.statusCode &&
        (httpBinding.statusCode < 100 || httpBinding.statusCode > 599)
      ) {
    ...
    case "amqp": {
      ...
      if (
        amqpBinding.deliveryMode &&
        ![1, 2].includes(amqpBinding.deliveryMode)
      ) {
        warnings.push(
          "AMQP delivery mode should be 1 (non-persistent) or 2 (persistent)",
        );
      }
      if (
        amqpBinding.priority &&
        (amqpBinding.priority < 0 || amqpBinding.priority > 255)
      ) {
 ...
 */
export function hasProtocolBinding(
  context: DecoratorContext,
  target: Operation | Model,
): boolean {
  const protocolMap = context.program.stateMap($lib.stateKeys.protocolConfigs);
 ...
 */
export function getAllProtocolConfigs(
  context: DecoratorContext,
): Map<Type, ProtocolConfig> {
  return context.program.stateMap($lib.stateKeys.protocolConfigs) as Map<
    Type,
    ProtocolConfig
  >;
}
