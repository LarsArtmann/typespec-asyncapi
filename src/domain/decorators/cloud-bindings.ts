...
export function $bindings(
  ...
  Effect.log(
    `🔗 PROCESSING @bindings decorator on ${target.kind}: ${target.name}`,
  );
  ...
  Effect.log(
    `✅ Successfully stored ${bindingType} binding for ${target.name}`,
  );
 ...
 */
function isValidBindingType(
  bindingType: string,
): bindingType is CloudBindingType {
 ...
 * TODO: Use Effect.TS!
 */
function validateAwsSnsConfig(
  config: Record<string, unknown>,
): AwsSnsBindingConfig | null {
  ...
  if (config.attributes && typeof config.attributes === "object") {
    snsConfig.attributes = config.attributes as Record<
      string,
      {
  ...
  if (config.filterPolicy && typeof config.filterPolicy === "object") {
    snsConfig.filterPolicy = config.filterPolicy as Record<
      string,
      string | string[]
    >;
  }
 ...
 * TODO: Use Effect.TS!
 */
function validateAwsSqsConfig(
  config: Record<string, unknown>,
): AwsSqsBindingConfig | null {
  ...
  return {
    ...
    ...(config.messageGroupId
      ? { messageGroupId: config.messageGroupId as string }
      : {}),
    ...(config.messageDeduplicationId
      ? { messageDeduplicationId: config.messageDeduplicationId as string }
      : {}),
    ...(config.visibilityTimeoutSeconds
      ? { visibilityTimeoutSeconds: config.visibilityTimeoutSeconds as number }
      : {}),
  };
}
 ...
 * TODO: Use Effect.TS!
 */
function validateGcpPubsubConfig(
  config: Record<string, unknown>,
): GoogleCloudPubSubBindingConfig | null {
  if (!config.topic || typeof config.topic !== "string") {
  ...
  return {
    ...
    ...(config.subscription
      ? { subscription: config.subscription as string }
      : {}),
    ...(config.orderingKey
      ? { orderingKey: config.orderingKey as string }
      : {}),
    ...(config.attributes
      ? { attributes: config.attributes as Record<string, string> }
      : {}),
  };
}
...
function validateKafkaConfig(
  ...
  return {
    ...
    ...(config.cleanup
      ? { cleanup: config.cleanup as "delete" | "compact" | "compact,delete" }
      : {}),
    ...(config.retentionMs
      ? { retentionMs: config.retentionMs as number }
      : {}),
