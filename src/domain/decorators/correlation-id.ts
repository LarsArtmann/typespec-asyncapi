...
export function $correlationId(
  ...
  if (target.kind !== "Model") {
    Effect.log(
      `❌ @correlationId can only be applied to models, not ${target.kind as string}`,
    );
    return;
  }
  ...
  if (!config.location || typeof config.location !== "string") {
    Effect.log(
      `❌ correlationId config must include location field (JSONPointer string)`,
    );
    return;
  }
  ...
  const schemaConfig = validateCorrelationSchema(
  ...
  const correlationMap = context.program.stateMap(
    $lib.stateKeys.correlationIds,
  );
  correlationMap.set(target, correlationConfig);

  Effect.log(
    `✅ Successfully stored correlation ID config for model ${target.name}`,
  );
...
function isValidJSONPointer(pointer: string): boolean {
  ...
  return (
    validLocations.includes(pointer) ||
...
function validateCorrelationSchema(
  ...
  if (!validTypes.includes(type)) {
    Effect.log(
      `⚠️ Invalid correlation schema type: ${type}, defaulting to 'string'`,
    );
    return {
      type: "string",
      format: "uuid",
