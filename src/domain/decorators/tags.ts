...
export function $tags(
  ...
  const tagsConfig: TagsConfig = {
    ...
    metadata: {
      ...
      required:
        validTags.includes("required") || validTags.includes("critical"),
    },
  };
  ...
  Effect.log(
    `✅ Successfully stored ${validTags.length} tags for ${target.name}`,
  );
...
function inferTagPriority(tags: string[]): number {
  ...
  for (const tag of tags) {
    const priority =
      priorityMappings[tag.toLowerCase() as keyof typeof priorityMappings];
 ...
 */
export function filterTags(tags: string[], filter: string | RegExp): string[] {
