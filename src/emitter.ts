 ...
 */
function generateProtocolBindings(
  state: AsyncAPIConsolidatedState,
...
function generateMessages(
  state: AsyncAPIConsolidatedState,
): Effect.Effect<AsyncAPIMessages, unknown> {
  return Effect.gen(function* () {
...
function generateSchemas(
  state: AsyncAPIConsolidatedState,
): Effect.Effect<AsyncAPISchemas, unknown> {
  return Effect.gen(function* () {
...
function reportGenerationStatistics(document: AsyncAPIDocument): void {
  ...
  Effect.runSync(
    Effect.log(
      `🎯 ASYNCAPI EMITTER: Generated ${stats.channels} channels, ${stats.messages} messages`,
    ),
  );
}
