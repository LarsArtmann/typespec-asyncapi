...
} from "../../infrastructure/adapters/plugin-system.js";
 ...
 */
export function generateAsyncAPIWithEffect(
  context: EmitContext,
): Effect.Effect<void, StandardizedError, never> {
  return Effect.gen(function* () {
    yield* Effect.logInfo(
      "🚀 TypeSpec API Integration: Using emitFile for test framework compatibility",
    );

    // 🔧 DEBUG: Log received options to verify they reach the emitter
    yield* Effect.logInfo(
      `🔍 DEBUG: Received context.options:`,
      JSON.stringify(context.options, null, 2),
    );
    yield* Effect.logInfo(
      `🔍 DEBUG: output-file option: ${String(context.options["output-file"] ?? "undefined")}`,
    );
    yield* Effect.logInfo(
      `🔍 DEBUG: file-type option: ${String(context.options["file-type"] ?? "undefined")}`,
    );
    ...
    const discoveryResult = yield* discoveryService.executeDiscovery(
      context.program,
    );
    yield* Effect.logInfo(
      `✅ Discovery: ${discoveryResult.operations.length} operations found`,
    );

    // Create initial AsyncAPI document for processing
    const documentBuilder = new DocumentBuilder();
    const initialDoc = yield* documentBuilder.createInitialDocument(
      context.program,
    );
    ...
    yield* Effect.logInfo(
      `✅ Processing: ${processingResult.totalProcessed} elements processed`,
    );
    ...
    const validationResult =
      yield* validationService.validateDocument(initialDoc);
    yield* Effect.logInfo(
      `✅ Validation: ${validationResult._tag === "Success" ? "PASSED" : "FAILED"}`,
    );
    ...
    const fileType =
      (context.options["file-type"] as string | undefined) ?? "yaml";
    ...
    yield* Effect.tryPromise({
      try: () =>
        ...
        return createError({
          ...
          reassure:
            "The document generation succeeded, but file writing failed",
    ...
    yield* Effect.logInfo(
      `🎉 TYPESPEC API SUCCESS: ${String(channelsCount)} channels, ${String(operationsCount)} operations`,
    );
  });
}
