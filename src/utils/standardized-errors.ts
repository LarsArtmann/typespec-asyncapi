 ...
 */
export const failWith = (error: StandardizedError) => Effect.fail(error);
...
export const emitterErrors = {
   ...
   */
  compilationFailed: (reason: string, source?: string) =>
    createError({
      what: "TypeSpec compilation failed",
      reassure:
        "This is usually a syntax or type error in your TypeSpec source",
   ...
   */
  invalidAsyncAPI: (validationErrors: string[], document?: unknown) =>
    createError({
   ...
   */
  pluginInitializationFailed: (pluginName: string, error: unknown) =>
    createError({
   ...
   */
  invalidConfiguration: (configPath: string, reason: string) =>
    createError({
   ...
   * File system errors
   */
  fileSystemError: (operation: string, path: string, error: unknown) =>
    createError({
   ...
   */
  validationFailure: (errors: string[], context?: unknown) =>
    createError({
   ...
   */
  emitterInitializationFailed: (reason: string, emitter?: unknown) =>
    createError({
      what: "AsyncAPI emitter failed to initialize",
      reassure:
        "This is a configuration issue, not a problem with your TypeSpec code",
      ...
      escape:
        "Try using the default emitter configuration or report this issue",
...
const transformError = (
  ...
  if (error instanceof Error) {
    return createError({
      ...
      escape:
        "Check your input and try with simpler data OR report this error at https://github.com/LarsArtmann/typespec-asyncapi/",
      severity: "error" as const,
...
export const railway = {
   ...
   */
  fromError: (error: Error, context?: Record<string, unknown>) =>
    transformError(error, context),

  /**
   * Transform unknown error to StandardizedError
   */
  fromUnknown: (error: unknown, context?: Record<string, unknown>) =>
    transformError(error, context),
   ...
   */
  all: <T1, T2 extends readonly Effect.Effect<T1, StandardizedError>[]>(
    effects: T2,
  ) => Effect.all(effects),
 ...
 */
export const isStandardizedError = (
  value: unknown,
): value is StandardizedError => {
  return (
    typeof value === "object" &&
    value !== null &&
    "what" in value &&
