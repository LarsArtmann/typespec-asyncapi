...
export function validatePositiveInteger(
  ...
  if (value !== undefined) {
    if (!Number.isInteger(value) || value < 1) {
      ...
      warnings.push(
        `High ${fieldName.toLowerCase()} value may impact performance`,
      );
    }
  }
...
export const protocolValidationHelpers = {
  ...
  ): result is Extract<ProtocolValidationResult, { _tag: "valid" }> =>
    result._tag === "valid",
  ...
  isInvalid: (
    result: ProtocolValidationResult,
  ): result is Extract<ProtocolValidationResult, { _tag: "invalid" }> =>
    result._tag === "invalid",
   ...
   */
  getWarnings: (result: ProtocolValidationResult): string[] => result.warnings,
};
