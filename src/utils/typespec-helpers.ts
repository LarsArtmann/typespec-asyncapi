 ...
 */
function getStateMap<T>(
  program: Program,
  stateKey: symbol,
): Map<unknown, T> | undefined {
...
export function getOperationType(
  ...
  const operationTypesMap = getStateMap<string>(
    program,
...
export function getChannelPath(
  ...
  const channelPathsMap = getStateMap<string>(
    program,
 ...
 */
export function getAsyncAPIAction(
...
export function getServerConfigs(
  program: Program,
  namespace: Namespace,
): Map<string, ServerConfig> | undefined {
  if (!program.stateMap || typeof program.stateMap !== "function") {
    return undefined;
  }
  const serverConfigsMap = program.stateMap(
    $lib.stateKeys.serverConfigs,
  ) as Map<Namespace, Map<string, ServerConfig>>;
...
export function getAllServerConfigs(
  program: Program,
): Map<Namespace, Map<string, ServerConfig>> {
...
export function getMessageConfig(
  ...
  const messageConfigsMap = getStateMap<MessageConfig>(
    program,
    $lib.stateKeys.messageConfigs,
  );
...
export function getProtocolConfig(
  ...
  const protocolConfigsMap = getStateMap<ProtocolConfig>(
    program,
    $lib.stateKeys.protocolConfigs,
  );
...
export function getSecurityConfig(
  ...
  const securityConfigsMap = getStateMap<SecurityConfig>(
    program,
...
export function buildServersFromNamespaces(program: Program): ServersObject {
  ...
  Effect.log(
    `🔍 Building servers: found ${allServerConfigs.size} namespace(s) with server configs`,
  );

  for (const [namespace, serverConfigsMap] of allServerConfigs.entries()) {
    // Get namespace name, handle global namespace
    const namespaceName =
      namespace.name === "" || !namespace.name ? "Global" : namespace.name;

    // Defensive check: ensure serverConfigsMap is iterable
    if (!serverConfigsMap || typeof serverConfigsMap.entries !== "function") {
      Effect.log(
        `⚠️ Skipping namespace ${namespaceName}: serverConfigsMap is not iterable`,
      );
      continue;
    }

    Effect.log(
      `📡 Processing namespace: ${namespaceName} with ${serverConfigsMap.size} server(s)`,
    );
    ...
    for (const [serverName, config] of serverConfigsMap.entries()) {
      ...
      Effect.log(
        `🖥️  Adding server: ${qualifiedName} (url: ${config.url}, protocol: ${config.protocol})`,
      );
      ...
      servers[qualifiedName] = {
        ...
        description:
          config.description ?? `${namespaceName} ${serverName} server`,
      } as const;
    }
  }
