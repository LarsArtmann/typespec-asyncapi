 ...
 */
export const validateAsyncAPIObjectEffect = (
  spec: unknown,
): Effect.Effect<boolean, ValidationError> =>
  Effect.gen(function* () {
    if (!spec || typeof spec !== "object") {
      return yield* Effect.fail(
        new ValidationError("Invalid AsyncAPI specification format", spec),
      );
    }
    ...
    if (!document["asyncapi"]) {
      return yield* Effect.fail(
        new ValidationError("Missing required 'asyncapi' field", spec),
      );
    }

    if (document["asyncapi"] !== ASYNCAPI_VERSIONS.CURRENT) {
      return yield* Effect.fail(
        new ValidationError(
          `Invalid AsyncAPI version: ${String(document["asyncapi"])}, expected ${ASYNCAPI_VERSIONS.CURRENT}`,
          spec,
        ),
      );
    }
    ...
    if (!document["info"]) {
      return yield* Effect.fail(
        new ValidationError("Missing required 'info' field", spec),
      );
    }
 ...
 */
export function isValidAsyncAPIStructure(
  document: unknown,
): document is AsyncAPIObject {
  if (!document || typeof document !== "object") return false;

  const doc = document as Record<string, unknown>;
  return (
    doc["asyncapi"] === ASYNCAPI_VERSIONS.CURRENT &&
 ...
 */
export function validateAsyncAPIObjectSync(document: unknown): {
  valid: boolean;
  errors: string[];
} {
  const errors: string[] = [];
  ...
  } else if (doc["asyncapi"] !== ASYNCAPI_VERSIONS.CURRENT) {
    errors.push(
      `Invalid AsyncAPI version: ${String(doc["asyncapi"])}, expected ${ASYNCAPI_VERSIONS.CURRENT}`,
    );
  }
