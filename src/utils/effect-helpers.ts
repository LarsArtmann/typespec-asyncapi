 ...
 */
export function executeEffect<T>(fn: () => Promise<T>): Effect.Effect<T, Error, never> {
...
export const effectUtils = {
   ...
   */
  fromResult: <A, E>(
    result: { success: true; data: A } | { success: false; error: E },
   ...
   */
  withLogging: <A, E>(effect: Effect.Effect<A, E>, operation: string): Effect.Effect<A, E> =>
    Effect.gen(function* () {
