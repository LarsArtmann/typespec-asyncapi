...
import type { AsyncAPIDocument, ChannelObject, OperationObject } from "../types/index.js";
import { getOperationType, getChannelPath, getAsyncAPIAction, sanitizeChannelId } from "./typespec-helpers.js";
import { convertModelToSchema } from "./schema-conversion.js";
 ...
 */
export function generateAsyncAPIContent(document: AsyncAPIDocument, fileType: "json" | "yaml", sourceInfo?: { sourceFiles?: string; operationsFound?: string }): string {
  let content: string;
...
export function logProcessingStats(document: AsyncAPIDocument, operations: Operation[]): void {
  ...
  console.log(`  - Channels created: ${Object.keys(document.channels || {}).length}`);
  console.log(`  - Operations created: ${Object.keys(document.operations || {}).length}`);
  console.log(`  - Schemas generated: ${Object.keys(document.components?.schemas || {}).length}`);
  console.log(`  - Messages generated: ${Object.keys(document.components?.messages || {}).length}`);
}
