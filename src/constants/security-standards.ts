 ...
 */
export const validateOAuth2Scheme = (
  scheme: unknown,
 ...
 */
export const validateSaslMechanism = (
  mechanism: string,
): { valid: boolean; errors: string[] } => {
  const errors: string[] = [];
  ...
  if (
    ...
    errors.push(
      `Invalid SASL mechanism: ${mechanism}. Must be one of: ${IANA_SASL_MECHANISMS.join(", ")}`,
    );
  }
 ...
 */
export const validateHttpScheme = (
  scheme: string,
): { valid: boolean; errors: string[] } => {
  const errors: string[] = [];
  ...
  if (
    ...
    errors.push(
      `Invalid HTTP scheme: ${scheme}. Must be one of: ${IANA_HTTP_SCHEMES.join(", ")}`,
    );
  }
