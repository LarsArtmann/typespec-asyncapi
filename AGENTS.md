...
- **pnpm 11 ignores `pnpm.overrides` in package.json** (warns "no longer read"); transitive-dependency overrides live in `pnpm-workspace.yaml` as a top-level `overrides:` map. npm-style top-level `overrides` in package.json is ALSO ignored by pnpm (the root's existing `overrides` block is inert npm compat; npm-managed `website/video` is where it does apply). Security pin precedent: `fast-uri ^3.1.6` + `js-yaml ^4.3.2` in `pnpm-workspace.yaml` silence the Dependabot alerts Dependabot itself cannot bump (`security_update_not_possible` for transitive deps).
- **`website/` is part of the root pnpm workspace** (single `pnpm-lock.yaml`; `website/pnpm-workspace.yaml` and `website/pnpm-lock.yaml` were removed deliberately). Its `allowBuilds: sharp` lives in the root `pnpm-workspace.yaml`. Do not recreate local lockfiles/workspace files under `website/` — Dependabot's `/website` directory entry reads the workspace root lockfile.
...
- **Fleet review context (2026-09-19)**: this site was part of the 17-site vision-review sweep and follow-up fixes (a11y, og:image, canonicals). Fleet-wide context, review pipeline, and ops docs live in `~/projects/vision-review-agent/docs/activation/site-fleet-ops.md` and `docs/status/2026-09-19_16-50_a11y-audit-17-sites.md` in that repo.
