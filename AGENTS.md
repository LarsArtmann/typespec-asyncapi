...
- **`@jsonSchemaExtension(key, value)` (M6, 29th decorator):** arbitrary JSON Schema keywords on Model/ModelProperty/Union/Enum/Scalar. Declaration `extern dec jsonSchemaExtension(target: Model | ModelProperty | Union | Enum | Scalar, key: valueof string, value: valueof unknown)` in lib/main.tsp; impl in `src/extension-decorators.ts` (extracted to respect the 400-line limit — M8's `@extension` joins this file); state via `storeJsonSchemaExtension` (merge into `Record<string, unknown>` per target); applied in `applyMetadata()` (constraint-mapper.ts) via direct `getStateMap(program, stateSymbols.jsonSchemaExtensions)` read — applies to ALL 5 targets as inline keywords AND `$ref` siblings (before the `$ref` skip). Merge semantics: decorators execute bottom-up, so the OUTERMOST (first-listed) same-key application wins. Key validation: `/^[A-Za-z_][A-Za-z0-9_.-]*$/u` → `invalid-json-schema-extension-key` warning. Values arrive as plain JS from `#{}` literals; state is NOT in the consolidated AsyncAPIConsolidatedState (constraint-mapper reads the stateMap directly).
