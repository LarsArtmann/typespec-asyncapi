...
pnpm run test        # Run tests via vitest (1010 pass, 0 fail)
...
- **Coverage gate at 75%** per-file minimum (scripts/coverage-gate.ts). Coverage runs via `bun test --coverage` (NOT vitest) because Bun's native coverage captures dynamically-loaded `dist/*.js` files that vitest/istanbul/c8 can't instrument (the TypeSpec compiler loads the emitter from `dist/` via its virtual filesystem, bypassing vitest's module transform and V8's script tracking). The gate script remaps `dist/src/*.js` back to `src/*.ts` paths and merges coverage, preferring the higher-coverage entry. Average: ~97.3%.
...
- **Emitter (9 core files, split from original 831-line monolith):**
  - `src/emitter.ts` (65 lines) — `$onEmit` entry point, writes output file, handles `split-schemas` option
  - `src/schema-emitter.ts` (355 lines) — `AsyncAPISchemaEmitter` class extends `TypeEmitter<JsonSchema, AsyncAPIEmitterOptions>`, overrides `modelDeclaration` (emits `allOf` for inheritance, `discriminator` for polymorphic models), `union` (emits `oneOf` for all-Model variants, `anyOf` for mixed), `enum`, `intrinsic`, `scalar`, etc.
  ...
  - `src/schema-generator.ts` (49 lines) — `generateSchemas()` entry point, creates asset emitter and collects declarations
  - `src/extract-value.ts` (24 lines) — `extractValue()` narrows `EmitEntity<T>` discriminated union, filters `Placeholder<T>` lazy values
  - `src/stdlib-helpers.ts` (43 lines) — `isStdlibType()` and `collectAllStdlibNames()` utilities
  - `src/constraint-mapper.ts` (156 lines) — `applyConstraints()`, `applyMetadata()`, `resolveEncode()`: maps 16 TypeSpec stdlib constraint/metadata mappings to JSON Schema keywords via table-driven `CONSTRAINT_TABLE` (10 validation entries) + inline metadata. Validation keywords skipped on `$ref` schemas. `resolveEncode()` safely passes `@encode` data to `serializeValueAsJson` for correct example/default serialization.
  - `src/document-builder.ts` (183 lines) — `buildAsyncAPIDocument()` entry point; delegates to `src/builders/` for assembly. Handles `$ref` chain construction
  - `src/intrinsic-mapping.ts` (82 lines) — `intrinsicToSchema()` maps TypeSpec scalar names to JSON Schema types (~30 cases)
  - `src/schema-splitter.ts` (75 lines) — `splitSchemas()` extracts schemas into individual files, rewrites `$ref` pointers to external paths
- **Document Builders (`src/builders/`, 11 files):**
  - `src/builders/operation-discovery.ts` (157 lines) — discovers channel-decorated operations from TypeSpec namespace
  - `src/builders/message-builder.ts` (203 lines) — builds message objects with `$ref` registration
  ...
  - `src/builders/channel-builder.ts` (86 lines) — builds channel objects with address, messages, bindings
  - `src/builders/server-builder.ts` (45 lines) — builds server objects with host, protocol, variables, and namespace-scoped bindings
  ...
  - `src/builders/components-builder.ts` (260 lines) — builds reusable `components.*` (operationTraits, messageTraits, parameters, correlationIds, operation/message/server/channel bindings) and applies `$ref` references via 3-way binding dispatch
  - `src/builders/types.ts` (82 lines) — shared builder context and result types
  - `src/builders/_imports.ts` (20 lines) — re-exports from `@typespec/compiler` for tree-shakeable imports across builders
...
- `test/compliance/` — **AsyncAPI 3.1.0 spec compliance suite** (~272 tests across 18 files): document structure, schema types, $ref chain, servers/security, protocol bindings (all 22 protocols), operation reply, multi-message operations, defaultContentType, @doc propagation, constraint decorators (54 tests), info object fields (6 tests), polymorphism/allOf/oneOf/discriminator (15 tests), reusable components (41 tests: operationTraits, messageTraits, parameters, correlationIds, operationBindings, messageBindings, channelBindings), edge cases. All validated against official AsyncAPI 3.1.0 JSON Schema via `compileAndValidateOrThrow()`.
