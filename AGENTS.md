...
- **Coverage gate at 75%** per-file minimum (scripts/coverage-gate.ts). Coverage runs via `bun test --coverage` (NOT vitest) because Bun's native coverage captures dynamically-loaded `dist/*.js` files that vitest/istanbul can't instrument (the TypeSpec compiler loads the emitter from `dist/`, bypassing vitest's module transform). The gate script remaps `dist/src/*.js` back to `src/*.ts` paths and merges coverage, preferring the higher-coverage entry. Average: ~96.7%.
- **Duplication budget:** jscpd threshold ratcheted to 5% (`.jscpd.json`). Current baseline: 38 clones at 4.06% (down from Phase-1 end 44 / 4.61%). Helpers added across Phase-1 + Phase-2: `BuilderFn` type alias (`src/builders/types.ts`), `iterNamedTypes`/`returnModels<T>` generators (`src/builders/shared-utils.ts`), `validatedDecorator`/`validateNonEmptyString` HOFs + `readModelProperty` (`src/decorator-helpers.ts`), `returnConst`/`returnNone`/`refOrFallback`/`collectModelProperties`/`buildEnumSchema`/`intrinsicSchema` private methods (`src/schema-emitter.ts`), `pushFieldError` (`src/validation/binding-field-validator.ts`), `pushIssue`/`stringifyBindingVersion` (`src/validation/binding-validator.ts`), `appendToStateArray` generic + `KafkaSaslConfig`/`MqttLastWillConfig` types + domain-type re-exports (`src/state-writers.ts`/`src/state.ts`), `CommonMetadata`/`DocumentBody` mixin interfaces (`src/domain/models/asyncapi-document.ts`), `shouldSkip` helper (`src/builders/message-builder.ts`). Run `bun run duplicate` to see current state. **Pareto floor reached** at ~4% — remaining clones are TypeScript signature/structural patterns that resist clean consolidation without breaking semantics or the 5-param lint rule. See `docs/planning/2026-08-05_19-55_SUPERB-PHASE-2-DEDUPLICATION-PLAN.md` for attempted-but-abandoned refactors (`applyMessageDecorator`, `reportAndAbort`).
