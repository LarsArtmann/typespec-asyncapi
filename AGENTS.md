...
- **Duplication budget:** jscpd threshold ratcheted to 5% (`.jscpd.json`). Current baseline: 44 clones at 4.61%. Helpers added to reduce duplication: `BuilderFn` type alias (`src/builders/types.ts`), `iterNamedTypes` generator (`src/builders/shared-utils.ts`), `validatedDecorator` HOF (`src/decorator-helpers.ts`), `returnConst`/`returnNone`/`refOrFallback`/`collectModelProperties`/`buildEnumSchema` private methods (`src/schema-emitter.ts`), `pushFieldError` (`src/validation/binding-field-validator.ts`), `appendToStateArray` generic + `KafkaSaslConfig`/`MqttLastWillConfig` types (`src/state-writers.ts`/`src/state.ts`), `returnModels<T>` generic (`src/builders/shared-utils.ts`), `readModelProperty` (`src/decorator-helpers.ts`). Run `bun run duplicate` to see current state.
