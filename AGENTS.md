...
- **Use `pnpm` for package management and scripts**. TypeScript scripts run via `bun run` (NOT `tsx` — `tsx` requires real Node.js which is unavailable on NixOS where `node` is actually a Bun wrapper). Never use `pnpm` or `pnpm dlx`.
...
- **Lint/duplication tools are pinned devDependencies** (`oxlint` 1.78.0, `jscpd` 4.0.9 in package.json), resolving from `node_modules/.bin` — NOT from system/Nix packages. Before pinning, GitHub CI was broken (`command not found` on ubuntu-latest, which has neither tool) and the local lint gate broke whenever the host oxlint drifted versions.
...
- **Emitter (9 core files, split from original 831-line monolith):**
  ...
  - `src/schema-emitter.ts` — `AsyncAPISchemaEmitter` class extends `TypeEmitter<JsonSchema, AsyncAPIEmitterOptions>`, overrides `modelDeclaration` (emits `allOf` for inheritance, `discriminator` for polymorphic models), `modelInstantiation`/`unionInstantiation` (argument-derived declaration names), `unionDeclaration` (emits `oneOf` for all-Model variants, `anyOf` for mixed), `enumDeclaration`, `intrinsic`, `operationReturnType`, etc.
...
- **Decorators:** `lib/main.tsp` declares all **28 decorators** + `EmitterOptions` model for IDE autocomplete. Decorator count: 18 core + 10 reusable-component (5 definition on Namespace + 5 reference on Operation/Model).
...
- **Root config is `tspconfig.yaml` only.** A `tspconfig.json` existed alongside it (split-brain); deleted 2026-08-14. TypeSpec resolves the yaml first; do not recreate the json.
- **Golden file regeneration:** `bun run scripts/regenerate-golden.ts` regenerates `test/realworld/golden/livesession-xyd.json` after intentional fixture/output changes. The test header referenced this script long before it existed (created 2026-08-14).
...
- **Security scheme types match AsyncAPI 3.1 spec exactly:** Valid types are `apiKey`, `asymmetricEncryption`, `gssapi`, `http`, `httpApiKey`, `oauth2`, `openIdConnect`, `plain`, `scramSha256`, `scramSha512`, `symmetricEncryption`, `userPassword`, `X509`. NOT valid: `sasl` (use the specific mechanism as type: `plain`, `scramSha256`, etc.), `mutualTLS`, `external`, `oauthBearer`. **`apiKey` vs `httpApiKey` field rules (from the AsyncAPI 3.1 schema itself):** `apiKey` = `{type, in}` where `in` ∈ `"user"|"password"` ONLY (SASL-style; NO `name` allowed, additionalProperties false). `httpApiKey` = `{type, name, in}` all REQUIRED, `in` ∈ `"header"|"query"|"cookie"`. SASL schemes (`plain`/`scramSha256`/`scramSha512`/`gssapi`) accept `{type, description?}` only — no `username`/`password`/`mechanism` fields in the output schema. The old AGENTS claim that apiKey allows header/query/cookie was wrong (that was OpenAPI's shape). Locked by `test/domain/security-schemes.test.ts` (13 AJV-validated tests; replaced the five fantasy security suites, 3208 lines, that tested HOBA/VAPID/risk-based-auth clones).
...
- **Template instantiations (M15, `Page<User>` → `PageUser`):** The asset-emitter's namespace walk NEVER declares template instantiations (it skips template declarations via `isTemplateDeclaration`, and instantiations aren't namespace members). Without an explicit `emitTypeReference`, any `$ref` to them dangles. Naming mirrors the framework's `declarationName` (type-emitter.js): base name + each argument's recursively-resolved name with first letter capitalized (`PageUser`, `BoxInt32`, `PagePageUser`); anonymous/literal/union args → name `undefined` → inline. `declarationNameOf()` in `src/schema-ref.ts` mirrors this; `schemaNameForType()` is the public API. Anonymous models have `name === ""` (empty string, NOT undefined) — name guards must use falsiness, not `=== undefined`. `refOrFallback()` in `schema-emitter.ts` short-circuits on hand-built refs ONLY for non-instantiations (namespace-walked, declaration guaranteed); instantiations run `emitTypeReference` first so `modelInstantiation()`/`unionInstantiation()` creates the declaration. `operationReturnType` is overridden to emit op-return types (framework default returns none). `modelInstantiation` inlines indexed instantiations (`Record<K,V>` → `additionalProperties`) to avoid declaring junk like `RecordStringUser`. Unspeakable base instantiations (`extends BaseEvent<{...}>`) compose via INLINE `allOf` schema (not a dangling `$ref` to the template declaration). Message keys/payload names for op returns use `schemaNameForType` (`resolveMessageKey`, `resolveMessageInfo`) so `op read(): Page<User>` registers message `PageUser` → `$ref: "#/components/schemas/PageUser"`. Collisions (explicit `model PageUser` + `Page<User>`) emit `duplicate-schema-name` warning from `schema-generator.ts`. Locked by `test/compliance/template-instantiations.test.ts` (8 tests).
- **Dead TypeEmitter overrides removed (M15 cleanup):** the framework dispatches by exact method name from `typeEmitterKey` (`namespace`, `modelInstantiation`, `unionDeclaration`, `enumDeclaration`, `scalarDeclaration`, `operationDeclaration`, `tupleLiteral`, `modelPropertyLiteral`, ...). Overrides named `namespace`, `union`, `enum`, `scalar`, `tuple`, `operation`, `modelProperties`, `modelProperty`, `namespaceDeclaration` were DEAD CODE (never dispatched) and have been removed. When adding overrides, verify the method name matches a dispatch key in `asset-emitter.js:typeEmitterKey`.
- **Examples are a pnpm workspace (M5):** `pnpm-workspace.yaml` lists `examples/*`; each example declares `"@lars-artmann/typespec-asyncapi": "workspace:*"` which pnpm links as a DIRECTORY symlink into the example's `node_modules`. Any test/tool that walks `examples/` recursively MUST skip `node_modules` and `tsp-output` (following the symlink cycle gives ELOOP — fixed in `all-examples-validation.test.ts` and `semantic-ref-resolution.test.ts`). `pnpm run check-examples` compiles every example (0 diagnostics) and AJV-validates the emitted document; CI runs it after `verify`. `#{}` value literals require COMMAS between members (newline-separated is a parse error — unlike model properties). `@operationSecurity(#{name})` only attaches a `$ref` — the scheme must ALSO be declared via `@security` or the ref dangles (caught by the semantic $ref test). `@reply` second arg is a runtime expression (`"$message.header#/replyTo"`), not a channel address.
- **`@encodedName` wire renaming (M7):** core TypeSpec decorator (NOT declared in lib/main.tsp — comes from `TypeSpec.encodedName`). The schema emitter resolves wire names via `resolveEncodedName(program, prop, "application/json")` (`wireNameOf()` in schema-emitter.ts) for `properties` keys, `required` entries, and `discriminator` values. Components schemas always use the `application/json` encoding regardless of message contentType (schemas are shared across messages; JSON is the AsyncAPI default). MIME-subtype resolution (`application/merge-patch+json` → `application/json`) is handled inside `resolveEncodedName`. Locked by `test/compliance/encoded-names.test.ts` (5 tests).
- **fast-check property suite (M13):** `test/property/emitter-properties.test.ts` — 6 invariants (AJV always passes, consistent constraints, $ref resolution ~1/~0-aware, type/const shape, split-schemas graph, determinism) over randomly generated specs, 20 runs each, seed pinned via `FC_SEED` env (default 20260821). fast-check is a devDependency. When a property fails in CI: the reported `seed` + counterexample reproduces exactly; run `FC_SEED=<seed> pnpm exec vitest run test/property`. Generator gotchas: lowercase identifiers MUST be filtered against TypeSpec reserved words ("model"/"enum"/"op"/... cause parse-error throws, not assertion failures), and `fc.option` defaults to `nil: undefined` — pass `{ nil: null }` when the type says `| null`.
- **EFv1 containment guard (M14):** eslint `no-restricted-imports` bans `@typespec/asset-emitter` for all of `src/` EXCEPT the three-file schema seam (`schema-generator.ts`, `schema-emitter.ts`, `extract-value.ts`). The 11 document builders consume only plain `@typespec/compiler` types. If you need asset-emitter elsewhere, you are widening the seam — reconsider (ROADMAP.md "EFv1 containment" documents the v0.4.0 direct-AST exit plan and the quarterly openapi3 watch trigger).
- **`@extension("x-...", value)` (M8, 30th decorator):** AsyncAPI specification extensions, mirroring `@typespec/openapi` syntax. Targets: Namespace → document ROOT object (spread after assembleDocument in document-builder.ts), Operation → operation object (operation-builder.ts), Model → message object (`applyMessageExtensions` BuilderFn in message-builder.ts, keyed via resolveMessageKey). Keys MUST start `x-` (else `invalid-extension-key` warning); repeatable with merge (outermost same-key wins). State: `objectExtensions` symbol → `state.objectExtensions` in the consolidated state (unlike jsonSchemaExtensions which reads stateMap directly). Index signatures added to OperationObject, MessageObject, DocumentBody to allow x- keys. AJV ignores x- keys (spec extensions) so output still validates.
- **`@jsonSchemaExtension(key, value)` (M6, 29th decorator):** arbitrary JSON Schema keywords on Model/ModelProperty/Union/Enum/Scalar. Declaration `extern dec jsonSchemaExtension(target: Model | ModelProperty | Union | Enum | Scalar, key: valueof string, value: valueof unknown)` in lib/main.tsp; impl in `src/extension-decorators.ts` (extracted to respect the 400-line limit — M8's `@extension` joins this file); state via `storeJsonSchemaExtension` (merge into `Record<string, unknown>` per target); applied in `applyMetadata()` (constraint-mapper.ts) via `jsonSchemaExtensionMap(program).get(target)` — applies to ALL 5 targets as inline keywords AND `$ref` siblings (before the `$ref` skip). Merge semantics: decorators execute bottom-up, so the OUTERMOST (first-listed) same-key application wins. Key validation: `/^[A-Za-z_][A-Za-z0-9_.-]*$/u` → `invalid-json-schema-extension-key` warning. Values arrive as plain JS from `#{}` literals; state is NOT in the consolidated AsyncAPIConsolidatedState (constraint-mapper reads the stateMap directly).
...
- **Real-world testing gotchas (from `test/realworld/`):**
  ...
  - **Named unions:** `union Foo { a: A, b: B }` produces `oneOf` in JSON Schema output. Union member models (A, B) are emitted as separate component schemas. The named union is ALSO declared in `components.schemas` (emitGlobalNamespace declares every named type) and receives `@doc`/`@summary` as `description`/`title` — locked in as public contract by the "named union metadata propagation" suite in `test/compliance/polymorphism.test.ts`. Properties typed by the union inline the `oneOf` directly rather than `$ref`-ing the union schema.
- **`enum` is reserved in `#{}` value literals:** Cannot use `enum` as a property name (e.g., `variables: #{ broker: #{ enum: #[...] } }`). The server builder maps `values` → `enum` in output: use `values: #["a", "b"]` in config, and it appears as `enum: ["a", "b"]` in the AsyncAPI output.
- **AsyncAPI 3.1 security format:** The `security` field on operations and servers contains `$ref` pointers to `components.securitySchemes` entries (or inline SecurityScheme objects), NOT OpenAPI-style `{ schemeName: [scopes] }` maps. `@operationSecurity(#{ name: "jwt" })` produces `{ $ref: "#/components/securitySchemes/jwt" }`. `SecurityRequirement` type is `Ref | Partial<SecurityScheme>`.
- **Blockless namespace ordering:** `namespace Foo;` (blockless) MUST appear before any other declarations. Models defined before a blockless namespace cause `blockless-namespace-first` errors. Put namespace first, then models and operations inside it.
- **Decorator execution is bottom-up:** In TypeSpec, decorators closest to the declaration execute first. `@useChannelServer("a")` above `@useChannelServer("b")` above `op foo()` stores `[b, a]`, not `[a, b]`. This affects ordering of server refs, security requirements, and other multi-value state.
- **Build pipeline ordering:** `buildServers` must run BEFORE `attachChannelServerRefs` in `document-builder.ts`. Otherwise `ctx.servers` is empty when channel server refs are attached, and `channel.servers` is silently omitted.
- **`storeMessageConfig` stores full config:** The function stores the complete `MessageConfigData` object (including `schemaFormat` and `examples`), not just `contentType`/`description`/`title`. Previous version dropped `schemaFormat` and `examples` silently.
- **TypeSpec v1.14 `@visibility` requires enum members:** `@visibility("read")` (string literal) causes a compilation error in TypeSpec v1.14+. Use `@visibility(Lifecycle.Read)` (enum member reference) instead. The same applies to `Lifecycle.Create`, `Lifecycle.Update`, etc.
- **`storeProtocolConfig` extracted:** This function lives in `src/store-protocol-config.ts`, not `src/state-writers.ts`. Re-exported from `state-writers.ts` for backward compatibility. The extraction was necessary to keep `state-writers.ts` under 400 lines.
- **asset-emitter (EFv1) is terminal but slow to die:** `@typespec/asset-emitter`'s package description says "to be replaced by the new emitter framework" (EFv2), but Microsoft's own `openapi3`/`json-schema` emitters still use EFv1 (as of 2026-08). EFv2 (alloy-js based) targets source-code emitters, not data documents — never adopt it here. Our asset-emitter usage is confined to schema generation (5 files, ~490 lines); `generateSchemas()` is the sole seam. The plan (contain → monitor openapi3 migration → direct-AST rewrite in v0.4.0) lives in ROADMAP.md "EFv1 containment".
- **Competing emitter `tsp-asyncapi` (npm, by marvin-hsu):** AsyncAPI 3.0-schema-validated, direct-AST-traversal, published on npm with VitePress docs. We lead on: AsyncAPI 3.1 validation, reusable components/traits (they "will not do"), split-schemas multi-file output, `@typespec/versioning`, spec-generated binding validation (22 protocols). They lead on: npm distribution, docs site, property-based tests, `@jsonSchemaExtension`/`@encodedName`/`asyncapi-id`/`@dynamicChannel`/`@replyAddress`, stable template-instantiation names (`Page<string>` → `PageString`). v0.3.0 plan: publish to npm, steal their DX features, fix Kafka `@protocol` emission bug, rewrite fake-assertion test files.
- **Real-world GitHub repo testing (`test/realworld/`):**
  - **5 repos tested:** bterlson/typespec-todo (10⭐), livesession/xyd (724 lines, compiles cleanly), DanSnow/typespec-events, Azure EventGrid, milehimikey/typespec-asyncapi (competing emitter)
  - **Zero repos use `TypeSpec.AsyncAPI`:** Exhaustive GitHub search found NO repos using this emitter's decorators outside this project
  - **livesession/xyd compiles cleanly:** 724-line production codebase, 40+ models, pure data models, zero diagnostics. Used as golden file regression guard
  - **3 adapted fixture tests:** `test/realworld/adapted/` takes real models from GitHub repos, wraps them in AsyncAPI decorators, and validates output. Tests: bytes, union enums, property refs, @visibility, @minLength, nested $ref, arrays, int64
  - **Cross-emitter differences with milehimikey:** `@server` (positional vs `#{}`), `@message` (string vs record), `@correlationId` (ModelProperty vs Model target), `@header` (no args vs name+value)
