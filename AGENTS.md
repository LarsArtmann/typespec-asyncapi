...
- **Duplication budget:** jscpd threshold ratcheted to 2% (`.jscpd.json`). Current baseline: **10 clones at 1.00% / 1.05% tokens** (Phase-3 end; down from Phase-2 end 38 / 4.06%, original Phase-1 44 / 4.61%). Pareto floor ~1% reached — remaining clones are TypeScript signature patterns, structural interface members, and framework-required class method overrides. Phase-3 helpers added: `withMessage`/`MessageDecoratorFn`/`applyMessageDecorator` (`src/builders/message-builder.ts`, `src/builders/types.ts`), inline type-modifier imports (`src/builders/_imports.ts`), single-line decorator signatures (`src/minimal-decorators.ts`), arrow-form class method overrides (`src/schema-emitter.ts`). Run `bun run duplicate` to see current state. See `docs/status/2026-08-05_20-35_PHASE-3-DEDUPLICATION-STATUS.md` for the full Phase-3 refactor log including attempted-but-reverted extractions (`validateProtocol`, `lookupMessage`).
