    ...
    rules: {
      ...
      "@typescript-eslint/naming-convention": [
        ...
        // Effect.TS service patterns and AsyncAPI internals - allow PascalCase and UPPER_CASE with underscores
        {
          ...
          filter: {
            ...
            regex:
              "^(.*Service|.*Manager|.*Handler|.*Collector|.*Monitor|.*Factory|.*Utils|.*Live|__ASYNCAPI_.*)$",
            match: true,
          },
        },
      ...
      "no-restricted-syntax": [
        "error",
        {
          ...
          message:
            "🚨 BANNED: throw statements. Use Effect.fail() or Effect.die() instead for proper error handling in Effect.TS pipelines.",
        },
        {
        ...
        {
          selector:
            "AwaitExpression > CallExpression[callee.type='MemberExpression'][callee.property.name='json']",
          message:
            "🚨 BANNED: await response.json() without validation. Use @effect/schema for type-safe JSON parsing.",
        },
        {
          selector:
            "ConditionalExpression[test.type='BinaryExpression'][test.operator='=='][test.right.type='Literal'][test.right.raw='null']",
          message:
            "🚨 BANNED: value == null checks. Use Option.fromNullable() for null-safe operations.",
        },
        {
          selector:
            "ConditionalExpression[test.type='BinaryExpression'][test.operator='!='][test.right.type='Literal'][test.right.raw='null']",
          message:
            "🚨 BANNED: value != null checks. Use Option.isSome() for type-safe null checks.",
        },
      ],
