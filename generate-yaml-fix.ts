...
channels:
${Object.entries(document.channels || {}).map(([channelPath, channel]) => 
  `  ${channelPath}:
...
messages:
${Object.entries(document.messages || {}).map(([messageName, message]) => 
  `  ${messageName}:
...
components:
  schemas:
${Object.entries(document.components?.schemas || {}).map(([schemaName, schema]) => 
  `    ${schemaName}:
      type: "${schema.type}"
      properties:
